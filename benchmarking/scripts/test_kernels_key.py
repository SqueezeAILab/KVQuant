...
for l in range(0,32):
    ...
    scalingfactor = rangeval.float().cuda()
    ...
    q = torch.ones((1,num_heads,head_dim), dtype=torch.float).cuda()
    mul = torch.zeros((1, num_heads, N), dtype=torch.float).cuda() #TODO: support longer q seqlens

    lookup_table = torch.tensor(nf4_signposts).float().cuda()

    d[f'layer{l} q'] = q
    d[f'layer{l} kcache2'] = kcache2
    d[f'layer{l} mul'] = mul
    d[f'layer{l} lookup_table'] = lookup_table
    d[f'layer{l} scalingfactor'] = scalingfactor
    d[f'layer{l} zeropoint'] = zeropoint
    d[f'layer{l} N'] = N
    d[f'layer{l} rows2'] = rows2
    d[f'layer{l} cols2'] = cols2
    d[f'layer{l} start_rows'] = start_rows
...
for i in range(0,num_iters):
    quant_cuda.vecquant4matmul_nuq_perchannel_transposed_rope_mha_batched_fused(
        d[f'layer{j} q'],
        d[f'layer{j} kcache2'],
        d[f'layer{j} mul'],
        d[f'layer{j} lookup_table'],
        d[f'layer{j} scalingfactor'],
        d[f'layer{j} zeropoint'],
        d[f'layer{j} N'],
        d[f'layer{j} rows2'],
        d[f'layer{j} cols2'],
        d[f'layer{j} start_rows'],
        d[f'layer{j} vals2'],
        d[f'layer{j} N'],
