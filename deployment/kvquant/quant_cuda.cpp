...
void vecquant4matmul_nuq_perchannel_transposed_rope_mha_batched_fused_opt(
  ...
  vecquant4matmul_nuq_perchannel_transposed_rope_mha_batched_fused_opt_cuda(vec, mat, mul, lookup_table, kcachelen);
}
...
void vecquant4matmul_nuq_perchannel_transposed_rope_mha_batched_fused_opt2(
  ...
  const at::cuda::OptionalCUDAGuard device_guard(device_of(vec));
  vecquant4matmul_nuq_perchannel_transposed_rope_mha_batched_fused_opt2_cuda(vec, mat, mul, lookup_table, kcachelen, rows, cols, startrows, spmat, num_rows, num_threads, nnz);
}
...
void vecquant4matmul_nuq_perchannel_transposed_mha_batched_fused_opt2(
  ...
  const at::cuda::OptionalCUDAGuard device_guard(device_of(vec));
  vecquant4matmul_nuq_perchannel_transposed_mha_batched_fused_opt2_cuda(vec, mat, mul, lookup_table, vcachelen, rows, cols, startcols, spmat, num_rows, num_threads, nnz);
}
